...
if __name__ == "__main__":
    ...
    alipay = AliPay(
        # 沙箱里面的appid值
        appid="2021000117612822",
        # notify_url是异步的url
        app_notify_url="http://82.156.63.62/alipay/return/",
        # 我们自己商户的密钥
        app_private_key_path="/Mxhop/apps/trade/keys/private_2048.text",
        # 支付宝的公钥
        alipay_public_key_path="/Mxhop/apps/trade/keys/alipay_key_2048.text",  # 支付宝的公钥，验证支付宝回传消息使用，不是你自己的公钥,
        # debug为true时使用沙箱的url。如果不是用正式环境的url
        debug=True,  # 默认False,
        return_url="http://82.156.63.62/alipay/return/"
    )
